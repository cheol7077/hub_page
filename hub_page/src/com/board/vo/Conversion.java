...
public class Conversion {
	...
	public List<BoardVO> getThumbnail(List<BoardVO> list) {
		...
		for(int i=0; i<list.size(); i++) {
			...
			File f = new File("C:\\Users\\hihi\\Documents\\카카오톡 받은 파일\\img_crawler\\img_crawler\\file\\"+communityId+"\\"+boardId); //이미지 경로(분석용)
			
			File[] allFiles = f.listFiles();
			for (File file : allFiles) { // 폴더 안에 있는 이미지 확장명 검사해서 이미지 파일일 경우 thumbnailName 변수에 이미지 파일 이름을 넣음(첫번째 사진만)
			...
			path = "file1/"+communityId+ "/" + boardId + "/" + thumbnailName.getName();	//워크스페이스 사진 경로(출력용)
			thumbnail = "<img src =\'" +path +"'"+" alt='사진을 로드하지 못했습니다'  style='width:400px; height:400px'/>";	//썸네일 태그 붙이기
			list.get(i).setThumbnail(thumbnail);	//리스트에 썸네일 저장
			
			}catch(Exception e) {
				list.get(i).setThumbnail("사진이 없습니다");	//아무런 사진이 없을 경우 "사진이 없습니다" 라는 문자열 저장
			}
		}
	...
	public List<BoardVO> conversion(List<BoardVO> list) throws ParseException{
		...
		for(int i=0; i<list.size(); i++) {	//content 내용 가공
			...
			String contents[] = temp.split("::");	// 공백으로 나누어져 있는 이미지 하나씩 떼내어 배열에 저장
			temp = "";
		
			for(int j=0; j<contents.length; j++) {		//떼네어진 이미지URL 중복사진 처리위해 가공
			contents[j] = contents[j].replace("::","");		//혹시 모를 불필요한 :: 있으면 삭제
...
//			}
			if(contents[j].startsWith("http")&&((contents[j].endsWith("jpg")||(contents[j].endsWith("bmp"))||(contents[j].endsWith("gif"))||(contents[j].endsWith("png"))||(contents[j].endsWith("jpeg"))
					||(contents[j].endsWith("JPG"))||(contents[j].endsWith("JPEG"))||(contents[j].endsWith("BMP"))||(contents[j].endsWith("GIF"))||(contents[j].endsWith("PNG")) || contents[j].contains("ruliweb") || contents[j].endsWith("mp4") ))){
				//URL 끝 검사(이미지URL인지 검사)
				if(list.get(i).getOverlab().equals(""))	//만약에 현재 Overlab 변수가 공백이면.
